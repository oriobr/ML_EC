...
```{r}
tree_1 <-
  tree(as.formula(paste0("V", ncol(train_df), "~.")), data = train_df)
```
...
::: {dir="rtl"}
מכיוון שמשתני העמודות אינם מוגדרים באופן נקודתי קשה להגיד משהו ממשי על התוצאות. אולם אנו יכולים לראות את "תהליך קבלת ההחלטות" שהוביל לחיזוי.
:::
...
::: {dir="rtl"}
פרמטר העלות למורכבות. נקבע ע"י הCV.
:::

### i 

```{r}
cv_tree_1 %>% plot(type ="s")
...
df_cv<- cv_tree_1[1:3] %>% qDT() 

best_size <- df_cv[,size[dev==min(dev)]]

pr_tree_1 <-  prune.misclass(tree_1, best = best_size)
```
...
cv_tree_2 %>% plot(type ="s")

df_cv_2<- cv_tree_2[1:3] %>% qDT() 

best_size_2 <- df_cv_2[,size[dev==min(dev)]]

pr_tree_2 <-  prune.misclass(tree_1, best = best_size_2)
