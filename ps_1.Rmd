...
df_step1 <- ksmooth(x=get(type_df)$medianIncome,y=get(type_df)$medianHouseValue,kernel_type,bandwidth = h_n,x.points = unique(get(type_df)$medianIncome)) %>% qDT()
...
df_plot_ks<- ks_df[kind == "train"][,gro := paste("kernel =",kernel,",","h = ",h ),by=.(kernel,h)]
...
ks_df[,.(mse = fmean((y -  true_y)^2)),by=.(kernel,h,kind_MSE)] %>% dcast(kernel+h~kind_MSE,value.var="mse") %>% flextable()

ks_df[,.(mse = fmean((y -  true_y)^2)),by=.(kernel,h,kind)]  %>%
  ggplot(aes(x=h,y=mse))+geom_line(aes(linetype = kind,color = kernel))+theme_linedraw()
