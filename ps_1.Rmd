...
ks_df2[,kind_MSE:=paste0(kind,"_mse")]

ks_df2[,.(mse = fmean((y -  true_y)^2)),by=.(kernel,h,kind_MSE)] %>% dcast(kernel+h~kind_MSE,value.var="mse") %>% flextable()

ks_df2[,.(mse = fmean((y -  true_y)^2)),by=.(kernel,h,kind)]  %>%
